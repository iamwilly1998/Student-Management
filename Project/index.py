...
@main.route('/nhanvien/quan_ly_sinh_vien')
@login_required
@role_only('NHANVIEN')
...
@main.route('/nhanvien/them_hoc_sinh')
@login_required
@role_only('NHANVIEN')
def add_student():
    form = AddUserForm()
...
@main.route('/nhanvien/quan_ly_lop_hoc')
@login_required
@role_only('NHANVIEN')
...
@main.route('/nhanvien/them_lop_hoc', methods = ['GET', 'POST'])
@login_required
@role_only('NHANVIEN')
def add_classes():
    msg = None
...
#API
@main.route('/api/user_pending', methods = ["POST"])
@login_required
def pending():
    default_value = 1
    ...
    gender = int(request.form.get('gender', default_value))
    ...
    if birthdate:
        ...
        return jsonify({"response": "Birthdate is required"})
    acceptable = True
    error = ""
...
@main.route('/api/user_pending/<id>', methods = ["DELETE"])
...
@main.route('/api/validate_user/<id>', methods = ["POST"])
def validate_user(id):
    ...
    if users_pending and id in users_pending:
        ...
        session['pending_users'] = users_pending
    return jsonify(msg)
@main.route('/api/validate_user', methods = ["POST"])
def validate_all():
    user = []
    count = 0
    overview = {
...
@main.route('/api/non_class_student/<grade>', methods = ['GET', 'POST'])
def get_non_class(grade):
    ...
    for s in students:
        ...
        list.append(temp)
    return jsonify(list)
@main.route('/api/change_class/<class_id>', methods = ['PUT', 'DELETE', 'POST'])
def change_info(class_id):
    ...
    return jsonify(msg)
@main.route('/api/subject_teacher/<subject_id>')
...
@main.route("/api/teaching_plan/<teacher_id>", methods = ['POST'])
...
@main.route("/api/score_validate", methods = ["POST"])
def validate_score():
    data = request.json
...
@main.route("/api/change_password/<user_id>", methods = ["POST"])
def change_password(user_id):
    ...
    return jsonify(msg)
@main.route('/api/change_avatar/<user_id>', methods = ["POST"])
