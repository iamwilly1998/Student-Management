...
def subject_report(subject_id=None, semester_id=None):
    ...
    if subject_id and semester_id:
        ...
        plans = db.session.query(TeachingPlan).join(Class).filter(TeachingPlan.subject_id.__eq__(subject_id),
                                                                  Class.year.__eq__(score_semester.year)).all()
        if len(plans) == 0:
            return data
        print(plans)
        avg_final_results = []
        ...
        for plan in plans:
            ...
            for s in students_query:
                avg = 0
                sum = 0
                s_score = db.session.query(Score).join(TeachingPlan).filter(Score.student_id.__eq__(s.user_id),
                                                                            Score.semester_id.__eq__(semester_id),
                                                                            TeachingPlan.subject_id.__eq__(
                                                                                subject_id)).first()
